...
def metrics_loss(model, test_val_loader_loss, batch_size): 
    ...
    loss_sum=[]
    loss_sum2=[]
    for  one_list in test_val_loader_loss:
        one_list = one_list.cuda() 
     
        loss_get,l2_regu_get= model(one_list) 

        loss= loss_get.mean()#-((prediction_i - prediction_j)).sigmoid().log().mean()+l2_regu
        loss2= l2_regu_get.mean()#-((prediction_i - prediction_j).sigmoid().log().mean())
        loss_sum.append(loss.item())  
        loss_sum2.append(loss2.item())#.item()
...
def metrics_loss_bpr(model, test_val_loader_loss, batch_size): 
    start_time = time.time() 
    loss_sum=[]
    loss_sum2=[]
    for user, item_i, item_j in test_val_loader_loss:
        ...
        loss_get,l2_regu_get= model(user, item_i, item_j) 
